...
def run():
    ...
    with sync_playwright() as p:
        ...
        # 1. Try connecting to existing Chrome (CDP) - SKIPPED to avoid hanging
        # logger.info("Attempting to connect to existing Chrome browser (CDP)...")
        # try:
        ...
        #     logger.info("Falling back to launching a new persistent browser instance...")

        # 2. Fallback: Launch persistent context
        if not browser:
            try:
                ...
                logger.info("Launched new persistent browser instance.")
        ...
        try:
            ...
            if browser:
                # If we launched it (persistent context), we should close it.
                # If we connected via CDP, maybe we shouldn't?
                # But typically for a script run, closing is fine or just disconnecting.
                # For persistent context, close() saves data.
                try:
                    browser.close()
